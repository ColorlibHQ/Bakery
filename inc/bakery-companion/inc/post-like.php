...
add_action( 'wp_enqueue_scripts', 'bakery_sl_enqueue_scripts' );
function bakery_sl_enqueue_scripts() {

	wp_enqueue_script( 'bakery-post-likes', BAKERY_COMPANION_DIR_URL . '/js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'bakery-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'bakery' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_bakery_process_simple_like', 'bakery_process_simple_like' );
add_action( 'wp_ajax_bakery_process_simple_like', 'bakery_process_simple_like' );
function bakery_process_simple_like() {
	// Bakery
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'bakery' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Bakery
	if ( $is_comment == 1 ) {
...
// User Profile List
add_action( 'show_user_profile', 'bakery_show_user_likes' );
add_action( 'edit_user_profile', 'bakery_show_user_likes' );
function bakery_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'bakery' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'bakery' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
