...
class bakery_newsletter_widget extends WP_Widget {
...
'bakery_newsletter_widget',


// Widget name will appear in UI
esc_html__( '[ Bakery ] Newsletter Form', 'bakery' ),

// Widget description
array( 'description' => esc_html__( 'Add footer newsletter signup form.', 'bakery' ), )
);
    ...
    <form target="_blank" novalidate action="<?php echo esc_url( $actionurl ); ?>" id="mc-embedded-subscribe-form" method="post" class="form-group d-flex flex-row">
        ...
        <div class="col-autos">
            <div class="input-group">
                ...
                <input type="email" name="EMAIL" class="form-control input-email" placeholder="<?php esc_html_e( 'Enter email', 'bakery' ); ?>" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter email'" required>
            </div>
        </div>
        <button type="submit" class="bbtns submit-form"><?php echo esc_html__( 'Subcribe', 'bakery')?></button>
        <button type="submit" class="subscribe_btn_footer click-btn">
...
if ( isset( $instance[ 'title' ] ) ) {
	...
	$title = esc_html__( 'Newsletter', 'bakery' );
}
...
<p>
<label for="<?php echo esc_attr( $this->get_field_id( 'actionurl' ) ); ?>"><?php esc_html_e( 'Action URL:' ,'bakery'); ?></label>
<p ><?php esc_html_e( 'Enter here your MailChimp action URL.' ,'bakery'); ?></p>
...
} // Class bakery_newsletter_widget ends here


// Register and load the widget
function bakery_newsletter_load_widget() {
	register_widget( 'bakery_newsletter_widget' );
}
add_action( 'widgets_init', 'bakery_newsletter_load_widget' );