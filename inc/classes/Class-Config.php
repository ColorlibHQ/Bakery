...
final class Bakery {
	...
	private function setup() {
		...
		$enqueu = new Bakery_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->bakery_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'bakery_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'bakery', BAKERY_DIR_PATH . '/languages' );
		...
		add_image_size( 'bakery_widget_post_thumb', 100, 60, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'bakery-np-thumb', 60, 60, true );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'bakery' ),
            'social-menu'   => esc_html__( 'Social Menu', 'bakery' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = BAKERY_DIR_CSS_URI;
		$jsPath  = BAKERY_DIR_JS_URI;
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'bakery' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate bakery theme customizer
		$bakery_theme_customizer = new bakery_theme_customizer();
	}
	...
	public function set_elementor_flag() {
		update_option( 'bakery_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'bakery-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'bakery_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'bakery-elementor-default-style',  BAKERY_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->bakery_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'bakery_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'bakery-elementor-notice', BAKERY_DIR_JS_URI.'bakery-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'bakery-elementor-notice',
				'bakeryElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'bakery-elementor-notice-nonce' ),
				)
			);
		}
